...
switch layerName
    ...
    case {'roughILMandISOS'}
        ...
        imgOld = img(:,2:end-1);     %还原到以前的图像
%             pathsTemp = getHyperReflectiveLayers(imgOld,params.roughILMandISOS);          %获得高反射率层        
%          hyj改
            pathsTemp1= getHyperReflectiveLayers_isos(imgOld,params.roughILMandISOS);
            pathsTemp2= getHyperReflectiveLayers_ilm(imgOld,params.roughILMandISOS);
        %save to structure 
        clear rPaths
        rPaths(1)= pathsTemp1;
        rPaths(2)= pathsTemp2;
       ...
       %%%%原来的程序%%%%%%%%%
%         pathsTemp = getHyperReflectiveLayers(imgOld,params.roughILMandISOS);         %获得超反射层        
%                         
%         %save to structure 
%         clear rPaths
%         rPaths = pathsTemp;
%         %
    ...
    case {'nflgcl' 'inlopl' 'ilm' 'isos' 'oplonl' 'iplinl' 'rpe' 'hyj' 'hyjj'}
...
% initialize region of interest   %初始化感兴趣区域
szImg = size(img);  
roiImg = zeros(szImg);

% avoid the top part of image   %把1--20行赋值0
roiImg(1:20,:) = 0;
...
for k = 2:szImg(2)-1
    ...
    switch layerName
        ...
        case {'nflgcl'}
            ...
            startInd0 = rPaths(strcmp('ilm',{rPaths.name})).pathX(indPathX(1));   %开始         
            indPathX = find(rPaths(strcmp('inlopl',{rPaths.name})).pathY==k);
            endInd0 = rPaths(strcmp('inlopl',{rPaths.name})).pathX(indPathX(1));   %结束
                        
            startInd = startInd0 - ceil(params.nflgcl_0*(endInd0-startInd0));  %ceil向上取整，round四舍五入，乘以缩放因子
        ...
        case {'rpe'}
            ...
            startInd = startInd0+round(params.rpe_0*(endInd0-startInd0));    %乘以缩放因子
...
%           endInd = endInd0 - params.oplonl_1;
         ...
         case {'hyj'}
            ...
            startInd = startInd0+round(params.hyj_0*(endInd0-startInd0));    %乘以缩放因子
            endInd = endInd0-round(params.hyj_1*(endInd0-startInd0));   
           ...
           case {'hyjj'}
            ...
            endInd0 = startInd0+round((rPaths(strcmp('rpe',{rPaths.name})).pathXmean-rPaths(strcmp('isos',{rPaths.name})).pathXmean));

            startInd = startInd0+round(params.hyjj_0*(endInd0-startInd0));    %乘以缩放因子
            endInd = endInd0+round(params.hyjj_1*(endInd0-startInd0));   
    end
